...
public final class ComputerService extends Service {
    ...
    @Override public int onStartCommand(Intent intent,int flags,int startId){
        ...
        if(!ACTIVE.compareAndSet(false,true)){
            progress("BUSY","上一项宿主操作尚未结束，请稍后重试");stopForeground(STOP_FOREGROUND_REMOVE);stopSelf();return START_NOT_STICKY;
        }
        ...
        task=executor.submit(()->{
            ...
            try(LocalComputer computer=new LocalComputer(this,this::progress)){
                ...
                running=false;
                String message=e instanceof InterruptedException?"操作已中断；磁盘和配置保留":e.getMessage();
                if(message==null)message=e.getClass().getSimpleName();
                try{
                    JSONObject cfg=new SecretStore(this).read();
                    String token=cfg.optJSONObject("computerIdentity")==null?"":cfg.getJSONObject("computerIdentity").optString("token");
    ...
    private static boolean relevantHostLine(String line){
        ...
        return s.contains("app.agbot.android")||s.contains("vminstance")||s.contains("crosvmbackendinstance")||
            s.contains("nativeprocess")||s.contains("crosvm  :")||s.contains("gunyah")||s.contains("gh-shim")||
            s.contains("gvswitch")||s.contains("bridgebackend")||s.contains("vm agbot computer");
    }

    private static void appendHostEvidence(StringBuilder out){
        out.append("\n--- host preflight ---\n");
        try{
            Shell.Result nodes=Shell.cmd("ls -l /dev/gunyah /dev/gunyah_share 2>&1; echo ---modules---; cat /proc/modules 2>/dev/null").exec();
            for(String line:nodes.getOut()){
                String lower=line.toLowerCase(Locale.ROOT);
                if(lower.contains("/dev/gunyah")||lower.contains("gunyah_host_share")||lower.startsWith("gh_")||lower.contains(" gunyah"))out.append(line).append('\n');
            }
            for(String line:nodes.getErr())if(line.contains("gunyah"))out.append(line).append('\n');
        }catch(Exception e){out.append("host-preflight-error=").append(e.getClass().getSimpleName()).append('\n');}

        out.append("\n--- relevant Android host log (latest) ---\n");
        try{
            Shell.Result logs=Shell.cmd("/system/bin/logcat -b all -d -v threadtime -t 6000").exec();
            ArrayDeque<String> keep=new ArrayDeque<>();
            for(String line:logs.getOut()){
                if(!relevantHostLine(line))continue;
                keep.addLast(line);
                while(keep.size()>500)keep.removeFirst();
            }
            for(String line:keep)out.append(line).append('\n');
            if(keep.isEmpty())out.append("(no relevant host log lines in latest 6000 lines)\n");
        }catch(Exception e){out.append("logcat-error=").append(e.getClass().getSimpleName()).append(':').append(e.getMessage()).append('\n');}
    }

    private static String sanitize(String text,String token){
        String value=text;
        if(token!=null&&!token.isEmpty())value=value.replace(token,"[REDACTED]");
        value=value.replaceAll("(?i)Bearer\\s+[A-Za-z0-9._-]+","Bearer [REDACTED]")
    ...
    public static String diagnostics(Context context) throws Exception {
        ...
        String token="";
        if(identity!=null){
            String id=identity.getString("id");java.util.UUID.fromString(id);token=identity.optString("token","");
        ...
        if(result.length()>1200000)result=result.substring(result.length()-1200000);
        return result;
    }

    @Override public void onDestroy(){
        cancelled=true;if(worker!=null)worker.interrupt();executor.shutdown();monitor.shutdownNow();
